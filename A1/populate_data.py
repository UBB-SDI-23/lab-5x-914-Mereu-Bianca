...
if __name__ == '__main__':
    ...
    with open('employees.sql', 'w') as file:
        ...
        sql = "TRUNCATE TABLE employees_projectemployee RESTART IDENTITY CASCADE;\n \
                ...
                ALTER TABLE employees_projectemployee DROP CONSTRAINT fk_employees_employee_id;\n \
                ALTER TABLE employees_projectemployee DROP CONSTRAINT fk_employees_project_id;\n \
                ALTER TABLE employees_employee DROP CONSTRAINT fk_employees_department_id;\n \
                DROP INDEX employee_department_id_idx;\n \
                DROP INDEX employeeproject_employee_id_idx;\n \
                DROP INDEX employeeproject_project_id_idx;\n"
        file.write(sql + "\n")

        for i in range(0, 1000000, 1000):
            departments = []
            for j in range(i, i + 1000):
                name = fake.random_element(elemnts=("Marketing", "Legal", "Administrative", "Development", "Leadership",
                                                    "Production", "Research and Development", "Accounting and Finance",
                                                    "IT", "Human Resources"))
                description = fake.santance()
                number_of_positions = fake.random_int(min=10, max=30)
                location = fake.random_int(min=1, max=1000)
        ...
        for i in range(0, 1000000, 1000):
            ...
            for j in range(i, i + 1000):
                ...
                status = fake.random_element(elemnts=("active", "vacation", "probation", "suspended", "retired"))
        ...
        for i in range(0, 1000000, 1000):
            ...
            for j in range(i, i+1000):
                ...
                employee_details.append(f"('{email}', '{birth_date}', '{phone_number}', '{address}', '{employee}')")
            data = f"INSERT INTO employees_employeedetail (first_name, last_name, email, city, date_of_birth) VALUES {','.join(employee_details)};"
            file.write(data + "\n")
        ...
        for i in range(10000):
            ...
            for j in range(1000):
                ...
                role = fake.random_element(elemnts=("Manager", "Team Leader", "Business Analyst", "Developer",
            ...
            data = f"INSERT INTO dogs_dogowner (employee, project, role, hours_worked) VALUES {','.join(employees_projects)};"
            file.write(data + "\n")
